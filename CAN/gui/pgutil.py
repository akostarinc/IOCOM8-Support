...
class ConfigLong:
    ...
    # Propagate non function properties to object
    def cpvars(self, copyto):
        for aa in dir(self):
            try:
                if aa[:2] != "__" and aa != "optarr" and  aa != "comline" and \
                                    aa != "printvars" and aa != "cpvars":
                    ff = getattr(self, aa)
                    if type(ff) == type(self.cpvars):
                        ff = "function"
                    else:
                        #print("setattr", aa, ff);
                        setattr(copyto, aa, ff)
            except:
                pass
...
def xmessage(strx, title = None, icon = Gtk.MessageType.INFO):
    ...
    if title:
        ...
        dialog.set_title("Akostar IOCOM CAN interface")
